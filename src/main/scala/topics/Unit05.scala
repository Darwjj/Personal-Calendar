...
//    println(i)
  ...
  @main def exercise54(): Unit =
    ...
    //Exercises
    lazy val a = List(1,2,3).foldLeft(0)(_+_)
    lazy val b = ???
    lazy val c = ???
    lazy val d = ???
    println(a)
  ...
  extension (ys: List[Int])
    def mymap(f: Int => Int): List[Int] = ???           // use foldRight
    def myfilter(p: Int => Boolean): List[Int] = ???    // use foldright
    def myreverse: List[Int] = ???                      // use foldLeft
