...
object Unit02 {
  ...
  */
  ...
  def findIndexOfSmallestTR(xs: scala.collection.mutable.ListBuffer[Int], from: Int): Int = {
    ...
    def loop(i: Int, s: Int): Int = {
      if (i >= l) s
        ...
        val newS = if (xs(i) < xs(s)) i else s
        loop(i+1, newS)
      }
    }
    loop(from, from)
  }
  def selectionSortTR(xs: scala.collection.mutable.ListBuffer[Int], p: Int = 0): Unit = {
    if (xs.nonEmpty) {
      val l = xs.length
      if (p < l) {
  ...
  @main def exerise23(): Unit =
    ...
    var nums2: ListBuffer[Int] = ListBuffer(8, 4, 0, 7, 3, 5, 1, 9, 6, -1)
    selectionSort(nums)
  ...
  def pwr(n: Num)(p: Num): Boolean = (n,p)match
    case (Zero,_) => false
    case (Succ(_), Zero) => true
    case (Succ(n), Succ(p)) => pwr(n)(p)
  ...
  @main def exercise26(): Unit =
    ...
    println(mul(toNum(4))(two))
    println(decr(ten))
    println(sub(three)(ten))
    println(pwr(one)(zero))
    println(pwr(zero)(one))
